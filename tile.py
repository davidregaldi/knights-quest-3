...
class Tile:
    def __init__(self, char, color=29, bold=False):
        self.char = char
        self.color = color
        self.bold = bold
...
def init_colors_extend(stdscr):
    ...
    for i, name in enumerate(rainbow_color_names, start=1):
        ...
        try:
            stdscr.addstr(i, 0, f"█ {i} {name} █", curses.color_pair(pair_number))
        except curses.error:
            continue
    curses.init_pair(31, 0, 10) # green health bar
    curses.init_pair(32, 0, 12) # blue mana bar
    curses.init_pair(33, 0, 3) # orange xp bar
    curses.init_pair(34, 7, 1) # rouge low health
    stdscr.clear()
